...
public-safe ingest configuration. The runtime MUST be implemented on top of the
Grafana Faro Web SDK while preserving a package-owned facade for consumers. The
runtime MUST support a disabled mode that still returns a usable no-op handle
for local development or intentionally disabled telemetry.
...
#### Scenario: Consumer initialization remains provider-hidden

- **WHEN** a browser app initializes the shared runtime with telemetry enabled
- **THEN** application bootstrap code can initialize observability without
  importing or configuring Grafana Faro directly
...
the same label shape. The package MUST own the translation from that normalized
contract into the Grafana Faro runtime so consumer code does not need to know
provider-specific metadata field names.
...
call even when telemetry is disabled, and the package MUST translate those
calls into Grafana Faro-backed telemetry without requiring provider-specific
wiring in consumer components.
...
those helpers mandatory for other consumers. The helpers MUST remain package
owned conveniences over the wrapper runtime rather than requiring direct
Grafana Faro setup in application code.
...
- **THEN** it can use package-provided React-friendly helpers instead of
  recreating runtime attachment logic or Grafana Faro setup in app-specific
  modules
...
context. The correlation contract MUST remain provider-neutral for consumers
even when the runtime is implemented on top of Grafana Faro.
